 ...
 * this route and only inverts on scroll, so the page has to open dark
 * whether or not there is a season to announce.
 *
 * The scrim held its density to the middle of the frame and faded from
 * there, which is the right shape for a photograph that is dark on the
 * right. This one is a white ground with red birds on it, and the copy block
 * runs to 71% of the width, so the headline, the eyebrow and the story all
 * sat in the fading half and measured between 1.8:1 and 2.0:1 against it.
 *
 * The stop moves to where the copy actually ends, and the density is tuned
 * against the render rather than by eye. Measured on the composited page
 * with the copy hidden, the eyebrow reads 5.4:1, the headline 5.2 and the
 * story 5.2. /93 cleared 10.6:1, which is a photograph half thrown away to
 * buy twice the contrast the copy needed.
 *
 * Full paper for the type, not the soft tint. `--color-paper-soft` is
 * documented in globals.css as text for the dark grounds, and it is right
 * there: over ink it clears 8.8:1. Over a photograph it is not a tint on a
 * known colour, it is a mid grey on whatever the picture happens to be, and
 * on this frame it measured 2.6:1. Buying that back with scrim alone takes
 * the overlay to 93% and leaves the photograph as a dark rectangle, so the
 * type gives up its tint instead. The hierarchy here is carried by size,
 * which is what a 10px letterspaced label and a 6rem headline already have.
 */
function PremiereHero({
  ...
  return (
    <section
      className={`relative isolate -mt-20 flex ${tall ? "min-h-[86svh]" : "min-h-[60svh]"} items-end overflow-hidden bg-ink pt-20`}
    >
      <Image
        src={image}
        alt=""
      ...
      <div className="shell relative z-10 flex max-w-2xl flex-col gap-7 pb-20 pt-28">{children}</div>
    </section>
  );
}
...
export default async function PremieresPage({
  ...
  // Between seasons, the day after a release with nothing written down yet,
  ...
  return (
    <>
      {next ? (
        <PremiereHero image={next.coverImage} tall>
        ...
        <PremiereHero image={featured.coverImage} tall={false}>
      ...
      <section className="shell reveal grid gap-14 py-24 lg:grid-cols-2 lg:gap-24">
        <div className="flex flex-col gap-6">
          <SectionHeading title={t("title")} lead={next ? t("lead") : t("leadBetween")} />
          {next ? (
        ...
        {next ? (
          ...
          <div className="flex flex-col gap-3 bg-paper-warm p-8">
            <h2 className="text-heading">{t("signupTitle")}</h2>
            <TextLink href="/contact">{t("betweenContact")}</TextLink>
          </div>
        )}
      </section>
